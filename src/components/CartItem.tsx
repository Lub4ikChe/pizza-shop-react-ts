...
import { minusPizza, plusPizza, removeItemFromCart } from '../redux/actions/cart';
import { IPizzaItemInCart } from '../redux/reducers/cart'

interface ICartItem extends IPizzaItemInCart {
    totalCount: number,
}

export const CartItem: React.FC<ICartItem> = ({ imageUrl, name, type, size, totalCount, price, id }) => {

    const dispatch = useDispatch();

    const hendleRemoveItem = (id: string): void => {
        if (window.confirm("Remove ? ")!) {
            dispatch(removeItemFromCart(id));
        }
    }

    const hendlePlusItem = (id: string): void => {
        dispatch(plusPizza(id));
    }

    const hendleMinusItem = (id: string): void => {
